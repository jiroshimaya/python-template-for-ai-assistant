...
class TestPrePushMainSync:
    def test_異常系_mainよりbehindならpushを失敗させる(self, tmp_path: Path) -> None:
    ...
    def test_正常系_mainよりaheadならpushを許可する(self, tmp_path: Path) -> None:
        result = _run_pre_push_hook(
            tmp_path=tmp_path,
            head_sha="bbbbbbbb",
    ...
    def test_異常系_divergedならpushを失敗させる(self, tmp_path: Path) -> None:
        result = _run_pre_push_hook(
            tmp_path=tmp_path,
            head_sha="cccccccc",
            origin_main_sha="bbbbbbbb",
...
def _run_pre_push_hook(
    ...
    *,
    ...
    script_path = repo_root / "scripts" / "pre_push_main_sync.sh"
...
""",
    ...
    return subprocess.run(
        ["bash", str(script_path), "origin", "git@github.com:jiroshimaya/repo.git"],
        text=True,
        capture_output=True,
        cwd=tmp_path,
        env=environment,
